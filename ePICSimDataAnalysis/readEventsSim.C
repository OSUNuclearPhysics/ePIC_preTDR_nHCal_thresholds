...
int readEventsSim(TString list = "data/filesSim_hcal_only_test_good.list", TString ofname = "output/output_test.root", long nevents = -1)
{
...
int MakeEvent(podio::EventStore *store, unsigned ev)
{
	...
	h_MCpart->Fill(MCParticles_store.size());

	h_MCpart_Pion_p->Fill(nPion_p);
	h_MCpart_Pion_n->Fill(nPion_n);
	h_MCpart_Kaon_p->Fill(nKaon_p);
	h_MCpart_Kaon_n->Fill(nKaon_n);
	h_MCpart_Proton_p->Fill(nProton_p);
	h_MCpart_Proton_n->Fill(nProton_n);
	h_MCpart_Electron_p->Fill(nElectron_p);
	h_MCpart_Electron_n->Fill(nElectron_n);

	h_MCpart_Neutron->Fill(nNeutron);
	...
	h_nHCal_nhits->Fill(nHCal_hits_store.size());


		for (unsigned hit_iter = 0; hit_iter < nHCal_hits_store.size(); ++hit_iter) {

			SimCalorimeterHit hit_nHCal =  nHCal_hits_store[hit_iter];
